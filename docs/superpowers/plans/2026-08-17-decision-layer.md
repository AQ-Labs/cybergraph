...
- `primary_reason` (in `decide`): the reason maximizing `(protected_boundary, effective_trust, impact_rank, reason_severity)` — **trust-first once protected status is equal**, not a fixed enum order (spec §4). `protected_boundary` (from whether the reason's route/entity is in the policy's `ProtectedSet`) is the top factor; `effective_trust` is `assurance.effective_trust(evidence, assurance)`. So a critical unsupported change on a protected boundary still leads, but among equally-(un)protected reasons a benchmark-backed `confirmed` finding leads over a lower-assurance "possible" one of higher raw impact — the collapsed headline must never bury a confirmed finding behind a vaguer one.
