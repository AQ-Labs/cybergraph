...
from cybergraph.security import triage as T
...
def test_should_suppress_only_on_grounded_false_positive():
    ...
    assert T.should_suppress(T.VERDICT_FALSE_POSITIVE, "validated by allowlist", slice_text)
    # false positive but evidence NOT in slice -> keep (faithfulness guard)
    assert not T.should_suppress(T.VERDICT_FALSE_POSITIVE, "sanitized upstream", slice_text)
    # false positive with empty/too-short evidence -> keep
    assert not T.should_suppress(T.VERDICT_FALSE_POSITIVE, "", slice_text)
    # non-FP verdicts never suppress
    assert not T.should_suppress(T.VERDICT_TRUE_POSITIVE, "validated by allowlist", slice_text)
    assert not T.should_suppress(T.VERDICT_UNCERTAIN, "validated by allowlist", slice_text)


def test_parse_verdict_defaults_to_uncertain_on_garbage():
    assert T._parse_verdict("not json at all")[0] == T.VERDICT_UNCERTAIN
    assert T._parse_verdict('{"verdict": "bogus"}')[0] == T.VERDICT_UNCERTAIN
    assert T._parse_verdict('{"verdict": "false_positive", "evidence": "x"}')[0] == T.VERDICT_FALSE_POSITIVE


def test_abstain_when_no_client_keeps_everything():
    results = T.triage_findings(Path("."), findings=[_finding()], client=None)
    assert len(results) == 1
    assert results[0].suppressed is False
    assert results[0].verdict == T.VERDICT_UNCERTAIN
...
def test_findings_load_and_grounded_fp_is_suppressed(tmp_path: Path):
    ...
    sup = T.triage_findings(repo, findings=findings, client=_Client(T.VERDICT_FALSE_POSITIVE, "select"))
...
def test_recall_guard_keeps_findings_on_ungrounded_or_uncertain_verdicts(tmp_path: Path):
    ...
    halluc = T.triage_findings(repo, findings=findings, client=_Client(T.VERDICT_FALSE_POSITIVE, "zzzz_not_present"))
    assert not any(r.suppressed for r in halluc)

    # uncertain and true_positive -> nothing suppressed.
    unc = T.triage_findings(repo, findings=findings, client=_Client(T.VERDICT_UNCERTAIN))
    tp = T.triage_findings(repo, findings=findings, client=_Client(T.VERDICT_TRUE_POSITIVE, "select"))
