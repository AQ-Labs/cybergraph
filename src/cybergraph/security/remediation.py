...
def remediation_for_sink(sink: str) -> str:
    ...
    if any(token in lowered for token in ("exec", "shell", "command", "subprocess", "process.start")):
    ...
    if any(token in lowered for token in ("open", "read", "write", "file")):
        return "Normalize the path, reject traversal, and enforce that resolved paths stay under an approved base directory."
    ...
    if "render_template_string" in lowered or "template" in lowered:
        return "Render trusted templates only and escape or strictly validate user-controlled template data."
    if "eval" in lowered:
        return "Remove dynamic evaluation of user-controlled strings and replace it with explicit dispatch."
...
def narrative_for_attack_path(path: AttackPath) -> str:
    source = ", ".join(path.taint_sources) if path.taint_sources else "no specific user input identified"
    data = "user-controlled data reaches the sink" if path.data_reachable else "the graph shows structural reachability"
    control = "A sanitizer or validation barrier appears on the path." if path.sanitized else "No sanitizer barrier was detected on the path."
...
def remediation_for_rule(rule_id: str, message: str = "") -> str:
    ...
    if "secret" in text:
        return "Move secrets to a managed secret store and prevent logging, response, or image-layer exposure."
